...
#include "frontend/impl/processor/depO3/depO3.h"
...
void DEPO3::init() {
  ...
  int max_trace_buf = param<int>("max_trace_buf").default_val(100000);
  ...
  int pipe_width   = param<int>("pipe_width").desc("Fetch and Retire Width").default_val(4);
  ...
  // Create the LLC
  m_llc = new DEPO3LLC(llc_latency, llc_capacity_per_core * m_num_cores, llc_linesize_bytes, llc_associativity, llc_num_mshr_per_core * m_num_cores, m_num_cores);
  if (llc_deserialize) {
  ...
  for (int id = 0; id < m_num_cores; id++) {
    ...
    DEPO3Core* core = new DEPO3Core(id, pipe_width, depth,
      m_num_expected_insts, m_num_max_cycles, active_list[active_id],
      cur_translate, m_llc, lat_hist_sensitivity, lat_dump_path, is_attacker, spec_type, max_trace_buf);
    core->m_callback = [this](Request& req){return this->receive(req);} ;
    m_cores.push_back(core);
  }

  m_logger = Logging::create_logger("DEPO3");
...
void DEPO3::receive(Request& req) {
  m_llc->receive(req);
...
void DEPO3::connect_memory_system(IMemorySystem* memory_system) {
  m_llc->connect_memory_system(memory_system);
};

int DEPO3::get_num_cores() {
  return m_num_cores;
};

DEPO3LLC* DEPO3::get_llc() {
  return m_llc;
}

std::vector<DEPO3Core*>& DEPO3::get_cores() {
  return m_cores;
}
