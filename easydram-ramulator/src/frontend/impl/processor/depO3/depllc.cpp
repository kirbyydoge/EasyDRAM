...
DEPO3LLC::DEPO3LLC(int latency, int size_bytes, int linesize_bytes, int associativity, int num_mshrs, int num_cores):
m_latency(latency), m_size_bytes(size_bytes), m_linesize_bytes(linesize_bytes), m_associativity(associativity), m_num_mshrs(num_mshrs) {
  m_logger = Logging::create_logger("DEPO3LLC");
  ...
  DEBUG_LOG(DDEPO3LLC, m_logger, "Index mask: {0:x}", m_index_mask);
  DEBUG_LOG(DDEPO3LLC, m_logger, "Index offset: {}",  m_index_offset);
  DEBUG_LOG(DDEPO3LLC, m_logger, "Tag offset: {}",    m_tag_offset);
}
...
bool DEPO3LLC::send(Request& req) {
  if (req.type_id == Request::Type::RowClone) {
  ...
  if (auto line_it = check_set_hit(set, req.addr); line_it != set.end()) {
    ...
    if (mshr_it != m_mshrs.end()) {
      DEBUG_LOG(DDEPO3LLC, m_logger,  "MSHR Hit.", m_clk);
    ...
    if (m_mshrs.size() == m_num_mshrs) {
      DEBUG_LOG(DDEPO3LLC, m_logger,  "No MSHR entry available.", m_clk);
    ...
    if (!line_available) {
      DEBUG_LOG(DDEPO3LLC, m_logger,  "No cache line available in the set.", m_clk);
      return false;
    }
...
DEPO3LLC::CacheSet_t& DEPO3LLC::get_set(Addr_t addr) {
...
DEPO3LLC::CacheSet_t::iterator DEPO3LLC::allocate_line(CacheSet_t& set, Addr_t addr) {
...
bool DEPO3LLC::need_eviction(const CacheSet_t& set, Addr_t addr) {
...
void DEPO3LLC::evict_line(CacheSet_t& set, CacheSet_t::iterator victim_it) {
  DEBUG_LOG(DDEPO3LLC, m_logger,  "Evicting {}.", victim_it->addr);
  s_llc_eviction++;
  ...
  if (victim_it->dirty) {
    ...
    DEBUG_LOG(DDEPO3LLC, m_logger,  "Writeback Request will be issued at Clk={}.", m_clk + m_latency);
  }

  set.erase(victim_it);
}

DEPO3LLC::CacheSet_t::iterator DEPO3LLC::check_set_hit(CacheSet_t& set, Addr_t addr) {
...
DEPO3LLC::MSHR_t::iterator DEPO3LLC::check_mshr_hit(Addr_t addr) {
  auto mshr_it =
    std::find_if(
      m_mshrs.begin(), m_mshrs.end(),
...
void DEPO3LLC::serialize(std::string serialization_filename) {
...
void DEPO3LLC::deserialize(std::string serialization_filename) {
  ...
  while (std::getline(serialization_file, file_line)) {
    ...
    if(m_cache_sets.find(index) == m_cache_sets.end()){
      m_cache_sets.insert({index, std::list<DEPO3LLC::Line>()});
    }
...
void DEPO3LLC::connect_memory_system(IMemorySystem* memory_system) {
...
int DEPO3LLC::get_mshrs_per_core() {
  return m_mshr_per_core;
}

int DEPO3LLC::get_blacklist_max_mshrs(int source_id) {
  return m_blacklist_max_mshrs[source_id];
}

void DEPO3LLC::set_blacklist_max_mshrs(int source_id, int max_mshr) {
  m_blacklist_max_mshrs[source_id] = max_mshr;
}

void DEPO3LLC::add_blacklist(int source_id) {
  m_blacklist_status[source_id] = true;
}

void DEPO3LLC::erase_blacklist(int source_id) {
...
bool DEPO3LLC::clflush(Addr_t addr) {
