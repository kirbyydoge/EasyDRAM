...
BHO3Core::Trace::Trace(std::string file_path_str) {
  ...
  std::unordered_set<std::string> support_reqs = {"RD", "WBACK", "ROWC"};
  while (std::getline(trace_file, line)) {
    std::vector<std::string> tokens;
    tokenize(tokens, line, " ");

    std::string& req_type = tokens[0];
    if (!support_reqs.count(req_type)) {
      throw ConfigurationError("Request type {} is not supported!", req_type);
    }

    int bubble_count = std::stoi(tokens[1]);
    Addr_t load_addr = std::stoll(tokens[2]);
    Addr_t store_addr = -1;
    bool is_rc = req_type == "ROWC";

    bool has_store = req_type != "RD";
    if (has_store) {
      store_addr = std::stoll(tokens[3]);
    }

    m_trace.push_back({bubble_count, load_addr, store_addr, is_rc});
  }
