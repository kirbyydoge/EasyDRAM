...
class ChronusDRAMController final : public IBHDRAMController, public Implementation {
    RAMULATOR_REGISTER_IMPLEMENTATION(IBHDRAMController, ChronusDRAMController, "ChronusDRAMController", "Chronus DRAM controller.")
...
public:
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override {
        ...
        // BreakHammer
        read_cycles = m_dram->m_timing_vals("nRAS") + m_dram->m_timing_vals("nRTP");
    ...
    void tick() override {
        ...
        bool is_prac_buffer_empty = m_prac_buffer.size() == 0;
        if (is_recovery_starting && is_prac_buffer_empty) {
            ...
            for (int i = 0; i < m_dram->get_level_size("rank"); i++) {
                m_rfmab_template->addr_vec[m_dram->m_levels("rank")] = i;
                m_prac_buffer.enqueue(*m_rfmab_template);
            }
        }

        auto prac_state = m_prac->get_state();
        if (prac_state == IPRAC::ABOState::RECOVERY && is_prac_buffer_empty) {
            for (int i = 0; i < m_dram->get_level_size("rank"); i++) {
                m_rfmab_template->addr_vec[m_dram->m_levels("rank")] = i;
                m_prac_buffer.enqueue(*m_rfmab_template);
            }
