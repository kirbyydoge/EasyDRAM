...
class mul_test extends BlackBox with HasBlackBoxResource
    with WithVCU108ClkIP
    with WithVCU108MulIP {
    val io = IO(new Bundle{
        val clk_sys_p = Input(Clock())
        val clk_sys_n = Input(Clock())
        val in0 = Input(UInt(16.W))
        val in1 = Input(UInt(16.W))
        val sel = Input(Bool())
        val inout_test = Analog(64.W)
        val out = Output(UInt(32.W))
    })
    addResource("/vsrc/mul_test.v")
}

trait WithVCU108ClkIP {
    ElaborationArtefacts.add(
        "vcu108clk.vivado.tcl",
    """
    create_ip -vendor xilinx.com -library ip -name clk_wiz -version 6.0 -module_name vcu108clk -dir $ipdir -force
...
trait WithVCU108MulIP {
    ElaborationArtefacts.add(
        "vcu108mul.vivado.tcl",
    """
        create_ip -vendor xilinx.com -library ip -name mult_gen -version 12.0 -module_name vcu108mul -dir $ipdir -force
        set_property -dict [list \
            CONFIG.CLK_INTF.FREQ_HZ         {10000000} \
            CONFIG.CLK_INTF.INSERT_VIP      {0} \
            CONFIG.CcmImp                   {Distributed_Memory} \
            CONFIG.ClockEnable              {false} \
            CONFIG.ConstValue               {129} \
            CONFIG.InternalUser             {0} \
            ...
            CONFIG.SclrCePriority           {SCLR_Overrides_CE} \
