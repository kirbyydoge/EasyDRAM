...
import easydram.{CanHavePeripheryEasyMemory, EasyDRAMTopIO}
import easydram.drambender.{DRAMBenderParams}
...
class WithEasyDRAMPunchthrough extends OverrideIOBinder({
  (system: CanHavePeripheryEasyMemory) => system.easydram_io.map { easydram_io =>
    val io_easymem_pins = IO(DataMirror.internal.chiselTypeClone[EasyDRAMTopIO](easydram_io))
    io_easymem_pins <> easydram_io
    (Seq(EasyDRAMPort(() => io_easymem_pins)), Nil)
  }.getOrElse((Nil, Nil))
})