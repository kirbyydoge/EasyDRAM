...
kprintf("Started Workload\n");
    ...
    for (uint64_t kib = 4; kib <= 32 * 1024; kib *= 2) {
        ...
        set_row(compatible_rows[0], SET_PATTERN);
        for (int i = 1; i < compatible_ctr; i++) {
            if (compatible_rows[2*i] != compatible_rows[2*i-1]) {
                set_row(compatible_rows[2*i], SET_PATTERN);
            }
        }
